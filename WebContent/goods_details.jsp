    ...
    <!-- <start header -->
       <form name="form1">
           <c:forEach items="${goodsList}" var="good" >
              <input type="hidden" name="img" value=${good.goodImg} />
              <input type="hidden" name="t1" value=${good.goodType} />
              <input type="hidden" name="c1" value=${good.goodColor} />
           </c:forEach>
        </form>
		<header>
...
<form action="GoodServlet?operate=buy" method="post" name = "form"> 
 <!--  <input type="hidden" id="operate" name="operate" value="buy"/> -->
  <input type="hidden" name="good_name" value="<%=goods.get(0).getGoodName() %>"/>
	<div class="xiangqing">
		<div class="neirong w">
			<div class="xiaomi6 fl"><%=goods.get(0).getGoodName() %></div>
			<nav class="fr">
				<li><a href="">概述</a></li>
				<li>|</li>
				<li><a href="">变焦双摄</a></li>
	...
	<div class="jieshao mt20 w">
		<div class="left fl"><img class="detailimg" id="photo" src=<%=goods.get(0).getGoodImg() %>></div>
		<div class="right fr">
			  <div class="h3 ml20 mt20"><%=goods.get(0).getGoodName() %></div>
			  <div class="jianjie mr40 ml20 mt10"><%=goods.get(0).getGoodDesc() %></div>
			  <div class="jiage ml20 mt10"><%=goods.get(0).getGoodPrice() %>元</div>
	    ...
	    <c:forEach items="${goodtList}" var="goods">
		        <div class="xzbb ml20 mt10 banbenb">
					<div class="banben fl">
						<a>
						    <input type="radio" id="type<%=count %>" name ="type" value=${goods.goodType} onclick="check()"/>
						    <label for="type<%=count %>" style="cursor:pointer">
								<span>${goods.goodType} </span>
								<span>${goods.goodPrice}元</span>
							</label>
						</a>
					</div>
			</div>
	    ...
	    <c:forEach items="${goodcList}" var="goods">
			<div class="xzbb ml20 mt10 colorc" id="${goods.goodColor}" >
				<div class="banben">
					<a>
					    <input type="radio" id="color<%=count2 %>" name="color" value="${goods.goodColor}" onclick="changeclo()" />
					    <label for="color<%=count2 %>" style="cursor:pointer">
							<span>${goods.goodColor}</span>
						</label>
					</a>
				</div>
			</div>
